...
namespace Capstone.Controllers
{
    ...
    public class PotatoDiseaseController : ControllerBase
    {
        private readonly PotatoModelHelper _modelHelper;
        private readonly ILogger<PotatoDiseaseController> _logger;

        public PotatoDiseaseController(PotatoModelHelper modelHelper, ILogger<PotatoDiseaseController> logger)
        {
        ...
        public async Task<IActionResult> Predict(IFormFile imageFile)
        {
            try
            {
                ...
                _logger.LogInformation("Processing potato disease prediction for file: {FileName}", imageFile.FileName);

                // Convert image to tensor
                var imageTensor = await ConvertImageToTensor(imageFile);
                
                var input = new PotatoInput
                {
                ...
                _logger.LogInformation("Potato disease prediction successful: {Disease} with {Confidence:P1} confidence", 
            ...
            {
                _logger.LogError(ex, "Potato disease prediction failed");
